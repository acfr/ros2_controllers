  ...
  const rclcpp::Time & time, const rclcpp::Duration & period)
{
    ...
    (std::abs(reference_interfaces_[2]) > 0.0349))
  {
    ...
    for (std::size_t i = 0; i < forward_states.size(); i++)
    {
      ...
      double current_steering = state_interfaces_[i + forward_states.size()].get_value();
  ...
  if (controller_state_publisher_->trylock())
  {
    ...
    for (size_t i = 0; i < number_of_drive_joints; ++i)
    {
      ...
      controller_state_publisher_->msg_.wheel_steer_position.push_back(
        state_interfaces_[number_of_drive_joints + i].get_value());
      ...
      controller_state_publisher_->msg_.wheel_steering_angle_cmd.push_back(
        command_interfaces_[number_of_drive_joints + i].get_value());
    }
  ...
  std::vector<double> & drive_values, std::vector<double> & steer_values)
{
  for (std::size_t i = 0; i < drive_joints_names_.size(); i++)
  {
    ...
    // then write
    command_interfaces_[i].set_value(drive_values[i]);
    command_interfaces_[i + steer_joints_names_.size()].set_value(steer_values[i]);
  }
  return;
}
...
bool SwerveController::check_joint_states_are_valid()
{
  ...
  const double fl_drive_joint_value = state_interfaces_[STATE_DRIVE_FL_WHEEL].get_value();
  const double fr_drive_joint_value = state_interfaces_[STATE_DRIVE_FR_WHEEL].get_value();
  const double rl_drive_joint_value = state_interfaces_[STATE_DRIVE_RL_WHEEL].get_value();
  const double rr_drive_joint_value = state_interfaces_[STATE_DRIVE_RR_WHEEL].get_value();

  const double fl_steer_joint_value = state_interfaces_[STATE_STEER_FL_WHEEL].get_value();
  const double fr_steer_joint_value = state_interfaces_[STATE_STEER_FR_WHEEL].get_value();
  const double rl_steer_joint_value = state_interfaces_[STATE_STEER_RL_WHEEL].get_value();
  const double rr_steer_joint_value = state_interfaces_[STATE_STEER_RR_WHEEL].get_value();
  ...
  for (std::size_t i = 0; i < icr_lines.size(); i++)
  {
    ...
    for (std::size_t j = 0; j < icr_lines.size(); j++)
    {
      if (j != i)
      {
        ...
        float abs_tol = 1e-15;
        float rel_tol = 1e-15;
        if (!is_close(result.z(), 0.0, abs_tol = 1e-5, rel_tol = 1e-5))
        {
