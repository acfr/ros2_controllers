...
namespace swerve_controller
{
  ...
  protected:
    controller_interface::CallbackReturn set_interface_numbers(size_t nr_state_itfs, size_t nr_cmd_itfs,
                                                               size_t nr_ref_itfs);
    ...
    struct WheelParams
    {
      ...
      double wheel_track = 0.0; // distance between left and right side wheels w.r.t. the midpoint of the wheel
    ...
    std::shared_ptr<rclcpp::Publisher<ControllerTwistReferenceMsg>> limited_velocity_publisher_ = nullptr;
    std::shared_ptr<realtime_tools::RealtimePublisher<ControllerTwistReferenceMsg>> realtime_limited_velocity_publisher_ =
        nullptr;
    ...
    rclcpp::Subscription<ControllerTwistReferenceMsg>::SharedPtr velocity_command_subscriber_ = nullptr;
    ...
    SWERVE_CONTROLLER_LOCAL void update_command_interfaces(std::vector<double> &drive_values,
                                                                 std::vector<double> &steer_values);

    SWERVE_CONTROLLER_LOCAL bool check_joint_states_are_valid();

    SWERVE_CONTROLLER_LOCAL void check_steering_limits(std::vector<DriveModuleDesiredValues> &result);
    ...
    SWERVE_CONTROLLER_LOCAL void reference_callback(const std::shared_ptr<ControllerTwistReferenceMsg> msg);
  };
} // namespace swerve_controller
