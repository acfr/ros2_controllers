...
set(_exported_dependencies "fmt;parameter_traits;rclcpp;rclcpp_lifecycle;rsl;tcb_span;tl_expected;control_msgs;control_toolbox;controller_interface;generate_parameter_library;geometry_msgs;hardware_interface;nav_msgs;pluginlib;rclcpp;rclcpp_lifecycle;rcpputils;realtime_tools;std_srvs;tf2;tf2_msgs;tf2_geometry_msgs;angles;visualization_msgs;ros2_controllers_interfaces")

find_package(ament_cmake_libraries QUIET REQUIRED)

# find_package() all dependencies
# and append their DEFINITIONS INCLUDE_DIRS, LIBRARIES, and LINK_FLAGS
# variables to swerve_controller_DEFINITIONS, swerve_controller_INCLUDE_DIRS,
# swerve_controller_LIBRARIES, and swerve_controller_LINK_FLAGS.
# Additionally collect the direct dependency names in
# swerve_controller_DEPENDENCIES as well as the recursive dependency names
# in swerve_controller_RECURSIVE_DEPENDENCIES.
if(NOT _exported_dependencies STREQUAL "")
  find_package(ament_cmake_core QUIET REQUIRED)
  set(swerve_controller_DEPENDENCIES ${_exported_dependencies})
  set(swerve_controller_RECURSIVE_DEPENDENCIES ${_exported_dependencies})
  set(_libraries)
  foreach(_dep ${_exported_dependencies})
    ...
    if(NOT "${${_dep}_TARGETS}" STREQUAL "")
      foreach(_target ${${_dep}_TARGETS})
        ...
        if(TARGET "${_target}")
          ...
          if(_include_dirs)
            list_append_unique(swerve_controller_INCLUDE_DIRS "${_include_dirs}")
          endif()
    ...
    if(NOT use_modern_cmake)
      if(${_dep}_DEFINITIONS)
        list_append_unique(swerve_controller_DEFINITIONS "${${_dep}_DEFINITIONS}")
      endif()
      if(${_dep}_INCLUDE_DIRS)
        list_append_unique(swerve_controller_INCLUDE_DIRS "${${_dep}_INCLUDE_DIRS}")
      endif()
      if(${_dep}_LIBRARIES)
        list(APPEND _libraries "${${_dep}_LIBRARIES}")
      endif()
      if(${_dep}_LINK_FLAGS)
        list_append_unique(swerve_controller_LINK_FLAGS "${${_dep}_LINK_FLAGS}")
      endif()
      if(${_dep}_RECURSIVE_DEPENDENCIES)
        list_append_unique(swerve_controller_RECURSIVE_DEPENDENCIES "${${_dep}_RECURSIVE_DEPENDENCIES}")
      endif()
    endif()
    if(_libraries)
      ament_libraries_deduplicate(_libraries "${_libraries}")
      list(APPEND swerve_controller_LIBRARIES "${_libraries}")
    endif()
  endforeach()
endif()
