...
#include "test_swerve_controller.hpp"
...
class SwerveControllerTest : public SwerveControllerFixture<TestableSwerveController>
{
};

// checking if all interfaces, command, state and reference are exported as expected
TEST_F(SwerveControllerTest, check_exported_interfaces)
{
  SetUpController();
  ...
  for (std::size_t i = 0; i < cmd_if_conf.names.size(); i++)
  {
    if (i < 4)
    {
    ...
    EXPECT_EQ(cmd_if_conf.names[i], joint_names_[i] + "/" + hw_interface_name_);
  }

  auto state_if_conf = controller_->state_interface_configuration();
  ASSERT_EQ(state_if_conf.names.size(), joint_state_values_.size());

  for (std::size_t i = 0; i < state_if_conf.names.size(); i++)
  {
    if (i < 4)
    {
      hw_interface_name_ = "velocity";
    ...
    EXPECT_EQ(state_if_conf.names[i], joint_names_[i] + "/" + hw_interface_name_);
  }
...
TEST_F(SwerveControllerTest, test_both_update_methods_for_ref_timeout)
{
  SetUpController();
